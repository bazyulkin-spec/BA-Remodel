...
## Шаг СБР — умный сброс по частям — ВЫПОЛНЕН

Запрос: вернуться к базовому не всё разом, а выборочно — только стены, только
точки, всё — и чтобы это было практично и не страшно.

Секция «Сброс» в конце «Комнаты», три ряда по смыслу:
- **Отделка**: «Узор» (поворот 0, смещение 0, привязка по центру — лечит
  «раскладка уехала»), «Отделка» (плитка, цвет, декор, зоны, панно, материал —
  геометрия цела), «Крашеное и наставленное» (переиспользован СУЩЕСТВУЮЩИЙ
  `resetPlacements` вместо дубля). Чип «Вид» не добавлялся: кнопка «вписать»
  на плане уже делает это, второй путь только путал бы.
- **Геометрия**: «Лишние точки» (`simplifyRoom` — углы остаются, точки на прямых
  уходят), «Выпрямить стены» (контур → прямоугольник по своим же габаритам;
  проёмы снимаются честно, потому что висели на номерах прежних стен).
- **Наставленное**: «Проёмы», «Мебель», «Ступени» — чипы появляются, только если
  соответствующее в комнате есть.
- **«Новый проект»** с подтверждением вторым тапом (чип превращается в «Нажми ещё
  раз — начнём заново») — диалога нет, случайно не сработает.

Главное свойство: КАЖДЫЙ сброс — обычный шаг с `pushUndo`, отмена возвращает как
было; об этом прямо написано в подсказке секции, чтобы нажимать было не страшно.

**Поймано при проверке:** три моих ключа (`reset_view`, `reset_all`,
`reset_all_confirm`) уже существовали с ДРУГИМ смыслом — старый сброс наставленного
в меню «…» и «Сбросить вид» в 3D. Дубли поймала проверка локалей; свои ключи
переименованы в `rst_*`, а вместо дубля функции переиспользован существующий
`resetPlacements`. Ровно тот случай, ради которого проверка и писалась.

Проверено: check.sh — восемь правил, 551 ключ ×4 локали без дублей, скобки честным
парсером, все восемь сбросов под отменой, смок «выпрямить стены» (габарит
сохраняется, площадь не уменьшается); ядро не менялось.
