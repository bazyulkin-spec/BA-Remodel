...
## Шаг УХ — четыре пункта фидбека: поле первым, удаление двери, «Поделиться», 3D-угол — ВЫПОЛНЕН

**1. Толщина «осталась по выбору»?** Нет — поле было, но стояло ПОСЛЕДНИМ в прокручиваемом
ряду и на планшете уезжало за край. Поля «Толщина» (Комната и per-wall в Поверхностях) и
«Высота стен» (3D) переставлены ПЕРВЫМИ, чипы-пресеты после них.

**2. Удаление/правка проёма.** Тап (без движения) по проёму на плане выбирает его:
сверху панель «{Тип} · стена N» с кнопками «Размер…», «Удалить», «✕». «Удалить» —
deleteSelectedOpening (проём + его тип, с undo). «Размер…» — мастер сразу на шаге размера,
ПРЕДЗАПОЛНЕННЫЙ текущими Ш/В/подоконником (OpeningWizard.editIndex; confirmOpeningWizard
ветка правки: замена Cutout и типа на месте, позиция сохраняется, «Вся стена» работает).
Механика: openingMovedFlag в OPENING_MOVE; выбор ставится в gestureEnd при тапе-без-
движения; openingSel сбрасывается в начале любого нового жеста; ряд чипов «+Дверь…»
скрыт, пока проём выбран.

**3. Шаринг раздельный и явный.** В «Отчёте» блок «Поделиться»: чип «Поделиться планом
(PNG)» — ТОЛЬКО картинка-схема; большая кнопка ниже — ТОЛЬКО PDF-отчёт. Ничего не
склеено. Правило 5b поймало неимпортированный Dim в ReportTab — добавлен.

**4. 3D «окно видно только под углом».** Корень: квад проёма смещался НАРУЖУ стены
(+n·0.008) и прятался за её видимой внутренней гранью — painter's algorithm отдавал стену
вперёд. Теперь смещение К КАМЕРЕ: знак по n·(cam−mid), величина 0.02 — обе ветки (активная
комната и соседние). Проём виден с любого угла с обеих сторон стены.

Строки op_edit/op_delete/share_title ×4. tools/check.sh — все проверки пройдены.

## Шаг ПР2 — практичность мастера: прикидка рядами, своё избранное, скрытые зоны — ВЫПОЛНЕН

Пользователь сформулировал четыре практических принципа работы.

**1. Прикидка «на вскидку».** На плане при включённых размерах — две янтарные пилюли:
сверху «≈N» (плиток влазит по ширине bbox) и слева «≈M» (по высоте); мастер перемножает
в уме — 7×5 ≈ 35. Шаг = плитка+шов, поворот кратный 90° учитывается свапом, прочие углы —
пилюли не показываются (прикидка честная только по сетке).

**2. Каталог не спамит.** Ряд встроенных пресетов размеров показывается ТОЛЬКО пока
избранное пустое: добавил свои размеры (★, до 8, хранятся глобально между проектами) —
каталог исчезает, остаются только свои. Добавление/удаление — чип ★ при текущем размере.

**3. Скрытые зоны под мебелью — генеральная фича, не костыль.** Мебель на плане (кухня,
шкаф, ванна) = зоны, где плитку не видно. VM: derived `hiddenTiles` (центр плитки внутри
прямоугольника мебели) + `hiddenCutNumbers`. Канвас 6f: скрытые плитки приглушены тёмной
заливкой (при показе мебели). «Обрезки»: зелёная строка «Под мебелью: N шт · №…» + совет
«ставь сюда некрасивые куски — их никто не увидит». Расчёт покупки честно не меняется —
плитки всё равно нужны; меняется КУДА класть некрасивое.

**4. Межкомнатные стены (ответ пользователю).** Да: каждая комната — свой контур,
раскладка режется по нему, под перегородку плитка не «затекает»; в дверном проёме —
пороги (шаг ПР). Возможное развитие — «сквозная» укладка с совмещением швов между
комнатами через проём.

Правило №7 уточнено: триггер только на joinToString С ЛЯМБДОЙ (ложное срабатывание на
joinToString(", ") поймано и устранено). Строки hidden_zone/hidden_zone_tip ×4.
tools/check.sh — все проверки пройдены.

## Шаг МТ — материалы: не только плитка (ламинат, паркет, доска) — ВЫПОЛНЕН

Ответ на вопрос «а можно не плитку, а паркет?». Ядро: `core/Material.kt` —
`MaterialKind { TILE, LAMINATE, PARQUET, DECK, NONE }`, расширение `isPlank`,
`MaterialSpec` (шаг разбега, минимальный концевой кусок, м²/штук в упаковке,
переиспользование остатков) с пресетами `preset` / `presetTile` / `presetPattern`:
ламинат 1200×190 в разбег ⅓, паркет 600×120 ёлочкой, доска 2000×145 с зазором 5 мм.
`PlankCalc.plan(...) → PlankPlan` считает ряды, планки с запасом, упаковки, процент
отхода, остатки по рядам, экономию от переиспользования, предупреждения о коротком
последнем куске и о совпадающих торцах. Ёлочка считается по площади (`estimated = true`).

Раскладка НЕ дублируется: планка — это узкий прямоугольник в том же `TilingEngine`,
поэтому чертёж, номера подрезки, 3D, AR, PDF и «Поделиться» работают без правок.
Материал живёт и у комнаты (`RoomDto.material`), и у зоны (`ZoneDto.material`) —
значит в одной комнате можно совместить плитку и паркет; `setMaterialKind` подставляет
пресет размера и узора. Строки `mat_*` / `plank_*` ×4 локали.

**Восстановлено 29 июля.** В архиве из репозитория файлов этого шага не оказалось:
`core/Material.kt` отсутствовал, в `Panels.kt` / `EditorViewModel.kt` / `ProjectDto.kt`
не было ни одного упоминания материала, ключей `mat_*` / `plank_*` в четырёх
`strings.xml` тоже не было. То есть в собранном APK материалов не было, хотя шаг
числился выполненным — этаж ЭТ доехал, а МТ нет. Код восстановлен по описанию выше,
контракт сохранён (имена типов, пресеты, поля `RoomDto.material` / `ZoneDto.material`).

Отход теперь два разных числа, их нельзя мешать: `wastePct` — отход раскладки
(распаковано минус уложено), `boughtM2` — закупка целыми упаковками. Расход считается
по той же сетке рядов, что рисует движок (сдвиг ряда 1/2, 1/3 или без сдвига), поэтому
числа в отчёте сходятся с номерами подрезки на чертеже; подрезки складываются в планки
как в контейнеры — это и есть переиспользование остатков.

Проверено: `tools/verify/VerifyPlank.kt` — 40 проверок, «ALL CHECKS PASSED» (kotlinc
2.1.0); `core/src/test/.../MaterialTest.kt` — 8 методов зелёные; старые харнессы
`Verify` и `VerifyV2` по-прежнему зелёные; `bash tools/check.sh` — все проверки пройдены.

## Шаг ЭТ — этажи: второй этаж и переход между ними — ВЫПОЛНЕН

Модель выбрана намеренно «дешёвая и безопасная»: активный этаж по-прежнему лежит в
`rooms`, остальные — в приватном `levelStore: Map<Int, LevelState>` (комнаты + мебель +
статусы работ). Поэтому НИ ОДИН из ~30 участков, где итерируются `rooms` (канвас, 3D,
привязки, hit-test, работы, итоги), не переписывался — они всегда видят один этаж.

VM: `activeLevel`, `levelNames`, `levelList`, `levelTitle`, `switchLevel`, `addLevel`
(копирует контуры текущего этажа, чтобы стены совпали; мебель и отметки работ — нет),
`deleteActiveLevel` (последний не удаляется), `renameActiveLevel`, `levelSummaries()`.
`stashLevels()` — единственная точка сборки всех этажей, только чтение, поэтому её
безопасно звать из composable. Этажи входят в undo (`Snap`) и в файл проекта
(`LevelDto` + `ProjectDto.levels/activeLevel`); файлы без `levels` грузятся как
одноэтажные. UI: чипы этажей над чипами комнат; в Отчёте — карточка «Этажи»
(площадь и штуки по каждому + итог). Строки `levels_title/level_n/add_level/
del_level/level_hint` ×4.

Инварианты проверены портом логики (переключение → правки → возврат → сохранение →
загрузка → удаление): комнаты не теряются и не дублируются, активный этаж всегда есть.

Правило №7 в `tools/check.sh` уточнено: окно открывается только для лямбды
joinToString, НЕ закрытой на той же строке (два ложных срабатывания на панели планки).

## Шаг СТ — ступени и крыльцо — ВЫПОЛНЕН

Ядро: `core/Stairs.kt`. `StairsSpec` (положение, ширина, число ступеней, проступь,
подступёнок, направление подъёма по 90°, `toLevel` — этаж или −1 для крыльца, своя
плитка и материал, флаг облицовки подступёнков). Габарит на плане НЕ вводится руками —
он выводится из числа ступеней и проступи (`runM`, `w`, `h`), поэтому лестница не может
разойтись с тем, что реально ляжет. `edge(i)` даёт линию стыка ступени — по ней
рисуются рёбра, `edge(-1)` — нижний край площадки.

`StairsCalc.plan(...) → StairsPlan`: плиток по ширине ступени и что срезаем с крайней,
куски на проступи (с учётом двух рядов у широкой ступени), подступёнки полосами из
целых плиток (как плинтус из плитки: `stripsPerTile` / `tilesForRisers`), площади
проступей и подступёнков, закупка с запасом. Три независимых проверки удобства:
формула 2h + b в 600–650 мм, проступь ≥ 250 мм, подступёнок 140–200 мм — они
независимы намеренно, потому что 2×220 + 200 = 640 формулу проходит, а ступень плохая.
`fitToHeight(h)` подбирает марш под подъём: держит подступёнок в диапазоне и правит
проступь по формуле — «ввёл 2.8 м, получил 17 ступеней».

Приложение: `RoomDto.stairs` + `RoomDto.outdoor`, состояние `stairs` / `outdoor` во VM
(в снимок отмены, в снапшот комнаты и в файл проекта), `Selection.Stair`,
`Drag.STAIR_MOVE` — тап по маршу выбирает, перетаскивание двигает (размеры цифрами,
как просил мастер). На плане: рамка марша, рёбра ступеней, стрелка подъёма и подпись
«Вверх, этаж 2» или «Крыльцо» — постороннему понятно без объяснений. Панель: секция
«Ступени и крыльцо» в разделе «Комната» (ширина, ступеней, проступь, подступёнок,
подъём с автоподбором, поворот 90°, облицовка подступёнков, «взять материал комнаты»,
куда ведёт, все результаты и предупреждения). Плашка выбранного объекта получила ветку
ступеней. Крыльцо (`outdoor`) снимает стены, потолок и плинтус — на улице их нет.
В Отчёте карточка «Ступени и крыльцо», в PDF — блок строк. Строки `stairs_*` ×4 локали.

**Проверено.** `core/src/test/.../StairsTest.kt` — 8 методов зелёные (габарит из ступеней,
рёбра от низа к верху при подъёме в обе стороны, куски и подрезка 209 мм на ступени
1 м плиткой 300×300, полосы подступёнков 1 против 2 при 160 и 140 мм, запас, три
проверки удобства, `fitToHeight` на 0.85 и 2.8 м, вырожденные данные). Вместе с
`MaterialTest` — 16 методов зелёные, харнессы `Verify` / `VerifyV2` / `VerifyPlank` тоже.

`tools/check.sh` получил **правило 8**: все варианты `Selection` должны быть покрыты в
`when (sel)` — иначе Kotlin 2.x не соберёт. Правило поймало реальную поломку этого шага
(новый `Selection.Stair` без ветки в плашке) и проверено негативным тестом: с убранной
веткой даёт «НЕ ПОКРЫТ Selection.Stair», с веткой — зелёное. Тело `when` берётся по
балансу скобок, иначе `else ->` из чужого `when` маскирует пропуск.

## Шаг 3Д — ступени в объёме и этаж-призрак снизу — ВЫПОЛНЕН

**Ступени в 3D.** Каждая ступень — свой массив от площадки: для ступени k рисуется
короб от пола до высоты (k+1)×подступёнок на её собственной глубине проступи, с учётом
направления подъёма (`dirDeg` 0/90/180/270 — четыре ветки `when`). Верхняя грань берёт
цвет плитки комнаты, боковые затенены как у мебели, поэтому марш читается как марш,
а не как ящик. Локальный `stairBox` повторяет форму `box` из блока мебели; общие
`addFace` / `shade` переиспользуются, никакой второй геометрии не появилось. Все вызовы
стоят до сортировки граней — иначе марш выпал бы из порядка отрисовки.

**Этаж-призрак.** VM: `showGhost` + `toggleGhost()` и `ghostRooms` — комнаты
БЛИЖАЙШЕГО этажа снизу (`levelStore.keys.filter { it < activeLevel }.maxOrNull()`),
пусто, если этажа нет или призрак выключен. На плане (`4g`, до контура активной
комнаты) — заливка почти прозрачная плюс пунктирная обводка: по ней выставляют стены
верхнего этажа. В 3D — те же контуры на `-wallHeightM` с `forceBack = true` (уходят за
всё остальное) и тонкие вертикальные связи по углам, чтобы читалось «это этаж ниже,
а не пятно под полом». Чип «Этаж снизу» появляется в разделе «Комната» только когда
этаж снизу есть — иначе он бы просто занимал место. Строка `ghost_below` ×4 локали.

Проверено: `tools/check.sh` — все восемь правил зелёные, ключи строк согласованы по
четырём локалям (447), скобки сбалансированы; ядро не менялось, тесты и харнессы
остаются зелёными.

## Шаг СТ2 — раскрой ступеней и марш на чертеже — ВЫПОЛНЕН

Две дырки предыдущего шага: марш не попадал ни в PNG, ни в PDF, а закупка считала
каждый краевой кусок отдельной плиткой.

**Раскрой.** `StairsCalc.cutPlan(spec) → StairsCut`: краевой кусок проступи (для ступени
1 м плиткой 300 срезаем 209, в дело идёт 91 мм), сколько таких кусков выходит из одной
плитки (три), сколько плиток на них нужно (одна на три ступени), остаток от плитки;
то же для полос подступёнков. `plan()` теперь берёт закупку из раскроя, а не из «каждый
кусок — плитка»: на том же марше 22 плитки вместо 24, и разрыв растёт, когда краевой
кусок мелкий. Раскрой виден в панели («Целых плиток», «Краевые куски: 3 × 91 мм · 1
плитка по 3»), в карточке Отчёта и строкой в PDF.

**Марш на чертеже.** Рисуется в `PlanShare` (секция `2s`), а не в `renderFloorBitmap` —
поэтому попадает и в PNG «Поделиться планом», и в PDF (у него та же картинка), но НЕ
попадает в текстуру пола 3D и в AR, где марш уже есть объёмом и нарисовался бы дважды.
На чертеже: габарит марша, рёбра ступеней, стрелка подъёма со стрелкой-наконечником,
слово «Вверх, этаж 2» или «Крыльцо» и оранжевая полоса краевого куска с его размером
в мм — заказчик и напарник видят, где и что режется.

**Проверено.** Новый тест `cuttingListPacksEdgePieces` (краевой кусок 91, три из плитки,
одна плитка на три ступени, 22 плитки к покупке; подступёнок 140 → 16; открытые
ступени → 0 плиток на подступёнки; ширина ровно под целые плитки → краевых кусков нет).
Прогон всех шести наборов тестов ядра — 38 методов зелёные (kotlinc 2.1.0),
`tools/check.sh` — восемь правил зелёные, 451 ключ строк согласован по четырём локалям,
аргументы форматирования совпадают.

Отдельно: харнесс тестов запускался на заглушке JUnit, у которой не было `assertNotNull`
и перегрузок с сообщением — компиляция падала, а прогон шёл на СТАРОМ jar и показывал
ложное «зелёное». Заглушка дополнена, jar пересобирается с нуля; при прогоне тестов
всегда проверять, что число методов совпало с ожидаемым.

## Шаг ДГ — дуги, круглые и овальные комнаты — ВЫПОЛНЕН

Ядро: `core/Arcs.kt`. Дуга живёт в контуре частой ломаной, потому что движок раскладки
работает с многоугольником — зато чертёж, подрезка, номера, 3D, AR и PDF получают дуги
без единой правки. Шаг ломаной берётся от размера плитки (`arcSegMm` = половина меньшей
стороны, 80…400 мм): под 600-ю плитку дробить контур на сантиметры незачем.

`radiusOf` / `arcLenOf` — радиус и длина по хорде и стрелке прогиба; `bend(a, b, sagitta)`
— промежуточные точки дуги, знак задаёт сторону, прогиб больше радиуса даёт дугу больше
полукруга; `circle` / `oval` — контур комнаты, число сегментов кратно четырём, поэтому
точки ложатся ровно на концы осей и габарит читается как ровные 3.00 м; `detectArcs` —
находит участки контура на одной дуге (подряд идущие рёбра с поворотом одного знака и
почти равной величины), `edgeInArc`, `maxTileOnArc` — совет по размеру плитки на радиусе.

Приложение: `makeRoundRoom(d)`, `makeOvalRoom(a, b)` (контур меняется целиком, проёмы и
толщины стен снимаются с предупреждением — они привязаны к номерам стен),
`bendSelectedWall(sagittaMm)` — гнёт стену, выбранную тапом по её метке размера; ключи
`wall-N` у дальних стен сдвигаются на число вставленных точек, у согнутой стены проём
снимается. `arcRuns` — производное состояние для подписей и плинтуса.

Подписи: вместо шестидесяти крошечных размеров дуга получает ОДНУ метку «R 1.45 · 3.90 м»
— и на экране, и в PNG «Поделиться планом» (значит и в PDF). Плинтус по дуге считается
одной длиной (`mergeArcSegments`), а не сорока кусками по 150 мм — гнутый плинтус
не пилят по сегментам ломаной. Панель: секция «Дуги и круглые комнаты» с диаметром,
осями овала, прогибом, кнопками «наружу»/«внутрь», списком найденных дуг и советом
по размеру плитки. Строки `arc_*` ×4 локали.

**Проверено.** Новый `ArcsTest` — 8 методов; тесты поймали два настоящих бага:
центр дуги ставился по ту же сторону, что прогиб (полукруг уходил вниз вместо вверх),
и участок дуги на замкнутом круге получался длиннее самого контура (n+1 рёбер при n).
Полный прогон ядра — 46 методов зелёные, харнессы `Verify` / `VerifyV2` / `VerifyPlank`
зелёные, `tools/check.sh` — восемь правил, 465 ключей строк согласованы по четырём
локалям (правило 4 поймало две строки, на которые я сослался раньше, чем добавил).

Известные границы: на согнутой стене проём приходится ставить заново; вершины дуги
заперты от перетаскивания (см. шаг УП) — дуга правится только целиком; дуга внутри автообведённого с фото контура может быть распознана там,
где мастер видел прямую стену.

## Шаг СМ — ступени без плитки, пол вокруг марша, дуга у смежной стены — ВЫПОЛНЕН

Три запроса мастера одной волной.

**Отделка ступеней.** `StairsFinish { TILE, WOOD, CONCRETE, NONE }` РАЗДЕЛЬНО для
проступи и подступёнка (`treadFinish` / `riserFinish`) — микс «проступь плиткой,
подступёнок деревом» это обычная жизнь, а не экзотика. Плитка считается раскладкой
и раскроем как раньше; дерево — доска на ступень с размером (панель показывает
«3 × 1000×300 мм»); бетон и «Только отметить» материала не требуют, но площадь
для справки остаётся. `cutPlan`/`plan` уважают отделку: `buyPieces` содержит только
плитку. В 3D верх ступени красится по отделке (плитка — цвет плитки, дерево — тёплый,
бетон — серый, отметка — тусклый), чтобы марш читался честно.

**Пол вокруг марша.** `StairsSpec.cutsFloor = true`: след марша уходит в раскладку
пола как вырез — подрезка плитки ВОКРУГ ступеней считается, нумеруется и видна на
чертеже/3D/PDF автоматически (движок уже умеет вырезы). То же в расчёте планки.
Чип «Пол под маршем» возвращает плитку под марш (лестница на этаж, под ней виден пол).
Сводка «купить по форматам» теперь включает плитку ступеней (раскрой без запаса,
общий запас добавляется один раз) — у мастера ОДНА цифра закупки, а не две.

**Дуга у смежной стены.** `bendSelectedWall` после сгиба активной комнаты ищет в
остальных комнатах ребро с теми же двумя точками (допуск 2 см, обе ориентации) и гнёт
его теми же точками дуги (для встречной ориентации — в обратном порядке), сдвигая
ключи `wall-N` проёмов/толщин соседа. Иначе между комнатами оставался бы серп ничьей
площади. Тост «Смежная стена соседней комнаты согнута той же дугой» ×4 локали.

**Аудит «как дизайнер»** (по коду, без запуска UI — это ограничение честно указано):
восемь сквозных сценариев (ступени↔отчёт, зоны↔счёт, марш за стеной, AR, пороги…);
проход «новичок и мастер» по панелям: подписи и единицы у всех полей, совет в каждом
предупреждении (найден и дополнен `stairs_tread_warn`), pushUndo у всех разрушающих
действий (одна ложная тревога — pushUndo после guard'ов, это правильно), чипы отделки
у обеих поверхностей. Найдена и исправлена реальная нестыковка: «купить по форматам»
не знал про плитку ступеней. Ядро: 47 методов зелёные, три харнесса зелёные,
`tools/check.sh` — восемь правил, 474 ключа строк ×4 локали, аргументы совпадают.

Известные границы (сказано мастеру): в AR марш виден вырезом в полу, объёма нет;
дуга у смежной стены ловится только когда точки стен совпадают (обычный случай
режима «Квартира»); правка дуги после создания — только отменой.

## Приёмка: когда это можно назвать финалом

Код в лучшей форме за всё время (47 тестов ядра, 3 харнесса, 8 правил линтера,
474 ключа ×4 локали), но финал объявляют зелёный билд в Actions и палец на экране —
ни того, ни другого в среде разработки нет. Чек-лист смоука на телефоне, ~15 минут:

1. Actions зелёный, APK ставится; СТАРЫЙ сохранённый проект открывается без потерь
   (новые поля имеют default'ы — но проверить глазами обязательно).
2. Круглая комната 3 м, плитка 300: раскладка живая, на дуге ОДНА метка «R … · … м»,
   плинтус в расчёте одной длиной, совет по размеру плитки виден.
3. Согнуть межкомнатную стену: тост про соседа, у соседней комнаты стена совпала,
   серпа между комнатами нет; проём на согнутой стене снялся с предупреждением.
4. Марш 3 ступени в комнате с плиткой: пол вокруг марша подрезался, номера кусков
   у края марша есть; чип «Пол под маршем» возвращает плитку под марш.
5. Проступь — дерево, подступёнок — плитка: в панели «Доски проступей 3 × 1000×300»,
   полосы подступёнков из плиток, «Купить» — только плитка; в «по форматам» одна цифра.
6. «Только отметить» с обеих сторон: марш на плане и в 3D есть, в закупке нет ничего.
7. «Подъём всего» 2.8 м → ~17 ступеней, формула 2h+b зелёная.
8. Отчёт и PDF: карточка ступеней с раскроем, марш на чертеже со стрелкой, словом
   и оранжевым краевым куском; «Поделиться планом (PNG)» — то же.
9. 3D: деревянный верх ступеней тёплый, бетонный серый; этаж-призрак виден под вторым
   этажом; поворот марша 90° ×4 — рёбра и стрелка следуют.
10. Ламинат в комнате + плитка в зоне: упаковки, отход, предупреждения о разбеге;
    иврит на новых экранах без обрезок и без русских строк.
11. Ловец падений (тихий): при обычных запусках НИКАКИХ диалогов и упоминаний сбоев;
    уронить в отладке → в «О программе» появилась строка «Отправить отчёт о сбое»,
    после отправки исчезла. Фото 100+ Мп как подложка — программа не падает.

Критерий: Actions зелёный + чек-лист пройден + один реальный объект без критических
замечаний = финал беты. Для релиза в Play остаётся Billing (отложен самим мастером).

## Шаг КР — ловец падений (тихий) и защита от OOM на фото — ВЫПОЛНЕН

**Правка по фидбеку мастера:** диалог «в прошлый раз программа упала» на старте —
это программа, сама объявляющая пользователю о своей сырости. Диалог УДАЛЁН.
Ловец работает невидимо: `CrashGuard.install` тихо пишет отчёт (модель, Android/SDK,
версия, поток, стек с обрезкой до 12k) и отдаёт падение системе. Строка «Отправить
отчёт о сбое» живёт ТОЛЬКО в «О программе» (`ProScreen`, под донат-картой) и видна
только когда отчёт реально есть (`peek`); после отправки исчезает (`clear`).
Пользователь, не видевший сбоя, не увидит и следа ловца. Строки `crash_send` /
`crash_quiet_note` ×4; `crash_title/body/close` удалены как следы диалога.

**Защита от OOM.** Реальный вектор «пользователь увидел баг»: фото с современной
камеры (50–200 Мп) декодировалось в полный размер — на слабом телефоне это
OutOfMemoryError и смерть процесса, runCatching его не ловит надёжно. Теперь
`decodeBitmap` ужимает до 2600 px по длинной стороне ПРЯМО НА ДЕКОДЕРЕ
(`setTargetSize`, на старых SDK — scaled bitmap): одна точка чинит все три загрузки
(подложка, текстура плитки, декор). Подложке и текстурам этого хватает с запасом.

Аудит «что ещё выдаёт сырость» по видимым строкам (маркеры бета/тест/скоро/WIP):
одна строка `soon` — из числа давно неиспользуемых (наследие потерянных файлов),
на экран не попадает. Хранилище проектов уже отказоустойчивое: битый JSON молча
пропускается, а не роняет список.

Проверено: check.sh зелёный, 477 ключей ×4 локали, скобки чистые. На устройстве
(чек-лист приёмки, п.11): уронить в отладке → строка появилась в «О программе»,
отправка ушла, строка исчезла; обычный запуск — никаких диалогов.

## Шаг РВ — ревизия по всей программе: найдено и вылечено — ВЫПОЛНЕН

Целевой проход по стыкам подсистем (зоны × ступени × вырезы × планка × отчёты),
по мусору и по мелочам. Найдено три настоящих дефекта и две косметики:

1. **Планка в зоне игнорировала вырезы и марши** — ламинат зоны считался поверх
   выреза в полу и под лестницей (и в активной зоне, и в `zonePlankPlans` отчёта).
2. **Раскладка зоны не знала про марши** — плитка зоны рисовалась и считалась под
   лестницей, хотя пол комнаты под маршем уже резался.
   Оба лечатся одним помощником `holesInside(x, y, w, h)`: вырезы комнаты + следы
   маршей с `cutsFloor`, пересекающие прямоугольник зоны — теперь их вычитают и
   плиточная раскладка зоны, и планка зоны, и планы зон для отчёта.
3. **Раскрой ступеней в Отчёте/PDF для марша без плитки** (дерево/бетон/отметка)
   давал строку «Раскрой N: Целых плиток 0». Нулевые части пропускаются, марш без
   плитки строки не даёт; нумерация маршей при этом сохраняется исходная.
4. Косметика: пустые сабтайтлы фолдов «Ступени и крыльцо» / «Дуги» → null
   (не рисовать пустой текст).
5. Ложная тревога собственного аудита: «дисбаланс скобок» в ProScreen — артефакт
   регэкспа (`//` внутри `https://…` принят за комментарий). Сырой файл сбалансирован
   (311/311, 68/68), правило 6 линтера зелёное. Урок в копилку: срезать комментарии
   регэкспом нельзя, если в строках бывают URL.

Проверено полной батареей: ядро kotlinc 2.1.0 — 47 методов зелёные; харнессы
`Verify` / `VerifyV2` / `VerifyPlank` зелёные; `tools/check.sh` — восемь правил;
476 ключей строк ×4 локали, аргументы форматирования совпадают; TODO/FIXME в коде
нет (одно слово «заглушка» — легитимный комментарий про рамку логотипа).

## Шаг УП — управляемость: замок дуги и быстрый старт — ВЫПОЛНЕН

Проход по программе глазами трёх ролей — мастер, дизайнер, новичок — с вопросом
«как это должно управляться». Два места, где управление было не «как нужно».

**Замок дуги.** Известная заноза: точки дуги были обычными вершинами — случайный
палец превращал ровную дугу в излом, лечившийся только отменой. Теперь
`vertexOnArc(i)` / `edgeOnArc(i)`: вершина считается дуговой, если любое из её рёбер
входит в `arcRuns` (концы дуги тоже заперты — их сдвиг даёт тот же излом). Захват
таких вершин пропускается (палец просто тащит комнату целиком), их ручки-кружки не
рисуются, «+» на серединах сегментов дуги не рисуется и не срабатывает. Прямые углы
комнаты остаются полностью редактируемыми. Логика проверена компилируемым портом
(kotlinc): согнутая комната 31 точка → 29 заперто, свободны ровно два прямых угла.
Остаточная граница (осознанно): числовой редактор длины СОСЕДНЕЙ прямой стены может
сдвинуть общий с дугой конец — это осмысленное действие с цифрой и отменой, не палец.

**Быстрый старт.** Новичку не с чего чертить, мастеру на объекте некогда: в «Комнате»
над списком комнат — ряд пресетов «Ванная 1.7×1.7 · Санузел 1.2×1.5 · Кухня 3×2.7 ·
Комната 4×3 · Коридор 1.2×4» (частотный порядок плиточника). Один тап — контур готов,
точные стороны дальше правятся цифрами как обычно; идёт через `replaceOutline`,
так что проёмы честно снимаются с предупреждением. Строки `room_presets`/`preset_*`
×4 локали.

Проверено: check.sh — восемь правил, 482 ключа ×4 локали, скобки чистые; ядро на
этом шаге не менялось (батарея 47 методов и три харнесса остаются зелёными с шага РВ),
замок дуги — отдельным компилируемым смоуком.

## Шаг ИЗ — свои размеры плитки вместо каталога — ВЫПОЛНЕН

Запрос мастера с живого скриншота (программа собралась и работает): каталожные
размеры не нужны — хочу только свои, хоть 2, хоть 10, и чтобы их можно было удалять.

Механика уже была наполовину: ★ добавляет текущий размер в избранное, и как только
есть хоть один свой — каталог прячется. Но удалить конкретный свой размер можно было
только окольно (применить его, потом снять ★), лимит был 8, и ничего из этого не было
объяснено — мастер до избранного просто не добрался. Доведено:

- **Долгое нажатие на своём размере удаляет его** — напрямую, без применения.
  Новый `FavChip` на стабильном API (`pointerInput` + `detectTapGestures`, без
  experimental): та же геометрия и цвета, что у `Chip`, хаптика на тап и на долгое.
  `removeFavTile(t)` в VM с персистом.
- **Лимит поднят 8 → 12** («может 2 или 3 или 10 — только свои»).
- **Две подсказки**: под каталогом (пока своих нет) — «Введи свой размер и нажми ★ —
  свои размеры заменят этот каталог. Хоть 2, хоть 12, только твои»; под своими —
  «Тап — применить · долгое нажатие — удалить. Удалишь все — каталог вернётся».
  Возврат каталога после удаления всех — уже существующее поведение, теперь названное.

Строки `fav_intro` / `fav_hint` ×4 локали. Проверено: check.sh — восемь правил,
484 ключа ×4, скобки чистые, все четыре импорта FavChip на месте; ядро не менялось.

## Шаг ЛН — линейка плитки: сколько в ряд и сколько рядов — ВЫПОЛНЕН

Запрос со второго живого скриншота: «не вижу линейку — сколько плиток влазит в ряд
и сколько в ширину». Прикидка «≈N × ≈M» в канве БЫЛА, но хоронила сама себя:
верхняя пилюля рисовалась ровно в точке зелёной метки «4.00 m», которая рисуется
ПОЗЖЕ и закрашивает её своим фоном; левая пилюля на `sx(minx) − 32d` уезжала за
край экрана, когда комната стоит у левого борта (ровно как на скриншоте мастера).
Классика: фича есть, увидеть нельзя.

Сделано: обе пилюли перенесены ВНУТРЬ комнаты, стопкой под верхним краем — там их
не перекрывает ничто и они не клипуются. Содержимое из «≈7» стало мастерским:
«В ряд: 6 целых + 385 мм» / «Рядов: 4 + 591 мм». Счёт честный: целые клетки,
попавшие в габарит целиком, С УЧЁТОМ ФАЗЫ узора (`pattern.offsetX/Y`), остаток —
всё, что уйдёт в подрезку с обоих концов, включая швы между целыми. Ёлочка
исключена (у неё нет рядов в этом смысле); поворот работает для 0/90/180/270 через
своп шагов. Включается той же кнопкой-линейкой в тулбаре (`showDims`).

Формула проверена смоуком: 4×3 м, плитка 600+3, фаза по центру → 6 целых + 385 мм
в ряд, 4 ряда + 591 мм; инвариант «целые×600 + швы + остаток = габарит» сходится
с точностью до миллиметров. Строки `ruler_across` / `ruler_rows` ×4 локали,
check.sh — восемь правил (5b поймало полное имя вместо импорта — приведено к стилю
проекта), 486 ключей, аргументы совпадают.

## Шаг МП — подсказка про полосу стала магнитом — ВЫПОЛНЕН

Запрос мастера: «плитка свободно елозит, как хочу; когда срабатывает подсказка —
подтягивается; но если двинул — ставит, как я хочу». Раньше подсказка про узкую
полосу умела только кнопку «Исправить» (прыжок), а магниты драга тянули лишь к
геометрии (центр, целая у стены). Теперь ЦЕЛЬ ПОДСКАЗКИ — сама магнит.

В `Drag.PATTERN` перед snapAxis считается фаза, при которой у больной стены
(худший THIN_STRIP из `cutReport`) остаётся ровно полплитки: `wall + step/2` по
нужной оси (вертикальная стена → ось X, горизонтальная → Y). Кандидат идёт в
snapAxis с kind = 1 и радиусом ×1.8 от обычного — самый сильный магнит. Семантика
мягкая, как и была: позиция каждый кадр считается от пальца, поэтому протащил за
радиус — отпустило и стоит, где палец; `anchor = FREE` — никто потом не двигает.
Направляющая магнита-подсказки рисуется НА БОЛЬНОЙ СТЕНЕ тёплым цветом (Warn),
геометрические — зелёным, как раньше; для этого `patternHintX/Y` в VM.

Смок (порт формулы): за радиусом — свободно, позиция = палец; в 8 мм от цели —
прилипла, кусок у стены ровно 302 мм (полплитки), полоса вылечена; радиус живёт
в экранных пикселях, поэтому при зуме ×3 сжимается втрое (11.5 см мира на общем
плане → 3.8 см вблизи) — грубое движение издалека, точное вблизи. Первая версия
смока сама ошиблась в масштабе допуска и упала — исправлен тест, не логика.
Кнопка «Исправить» остаётся для тех, кто не хочет возить пальцем.
check.sh — восемь правил зелёные; ядро не менялось.

## Шаг ОБ — проекты: постоянная подпись, экспорт/импорт/обмен — ВЫПОЛНЕН

Боль мастера: «новая версия станет, только если удалю старую» — а удаление стирает
все проекты (они в `filesDir/projects/*.json`). Причина: debug-сборки в CI
подписывались СЛУЧАЙНЫМ ключом каждого запуска — Android считает это другой
программой. Лечение в корне и по симптомам:

**1. Постоянный ключ подписи.** `keystore/debug.keystore` (RSA 2048, 30 лет,
пароли «android») закоммичен в репозиторий; `signingConfigs.debug` в
`app/build.gradle.kts` теперь берёт его. Все будущие сборки подписаны одинаково →
обновления ставятся ПОВЕРХ, проекты живут. Это ключ ТОЛЬКО для бета-раздачи;
в Play идёт release-ключ из секрета CI, как и было. Один последний переезд
со старой версии всё же неизбежен (её ключ утерян в природе CI).

**2. Проект — это файл.** В листе проектов три кнопки: «В файл» (SAF
CreateDocument, имя «Кухня.baremodel.json» — Диск, Загрузки, куда угодно),
«Из файла» (OpenDocument; битый/чужой файл — короткий отказ «Это не файл проекта»,
без падения; совпадение имени НЕ затирает существующий — суффикс « (2)»),
«Отправить проект» (ACTION_SEND через существующий FileProvider, cache/reports).
Подсказка под кнопками объясняет обмен. `ProjectRepository.encode/decode` наружу;
`ignoreUnknownKeys` уже стоял — старые файлы открываются новыми версиями.
Строки `proj_*` ×8 ×4 локали.

**3. Спасение уже существующих проектов старой версии** (внутреннее хранилище, но
сборка отладочная — Android даёт `run-as` без root):
```
adb exec-out run-as com.baremodel.app tar -c files/projects | tar -xv
```
или по одному: `adb shell run-as com.baremodel.app cat files/projects/ИМЯ.json > ИМЯ.json`.
Вытащенные .json открываются в новой версии через «Из файла». Без ПК с adb пути
нет — экспорт в старой сборке отсутствует, хранилище внутреннее.

Проверено: check.sh — восемь правил, 494 ключа ×4 локали, скобки честным парсером;
`activity-compose` в зависимостях есть (лаунчеры SAF); смок круговой JSON и логика
суффикса имени; ядро не менялось.

## Шаг ВН — план от внешнего помощника: применено выборочно — ВЫПОЛНЕН

Мастер принёс пошаговый план правок от другого ИИ (писан вслепую, без кода).
Разбор перед применением — обязательная практика: слепые инструкции ломают сборку.

**Применено (две реально полезные идеи):**
1. Шторка панели: 172/330 → 0/240/560 dp с tween 280 мс FastOutSlowInEasing.
   Ручка и заголовок живут ВНЕ ограничения высоты (проверено по структуре), поэтому
   0.dp в сложенном состоянии безопасен — контент и так спрятан Fade'ом; свайп
   доступен всегда. Развёрнутые 560 — чтобы на телефоне не скроллить каждую
   секцию по чуть-чуть.
2. Убран внезапный диалог по голому тапу на стене в комнатном режиме («что здесь?»).
   Он стал лишним: явный режим размещения проёма давно существует
   (`placeOpeningKind`, кнопки «+ Окно»/«+ Дверь» в панели и чипы типов в
   MainScreen) — теперь тап по стене не прерывает работу. Блок заменён комментарием,
   режим размещения цел (10 упоминаний, блок тапа-установки на месте).

**Отклонено из плана — с причинами:**
- «Изменения 3–4» (state placeOpeningKind, кнопка, выбор типа) — УЖЕ ПОСТРОЕНЫ
  в кодовой базе; план предлагал дубликат, причём без главного — без обработчика
  тапа, ставящего проём (кнопка взводила бы режим-пустышку).
- Строки: план клал русский текст в values/ (у нас это АНГЛИЙСКАЯ локаль по
  умолчанию) и ссылался на несуществующую values-en/ — упало бы на нашем же
  правиле 4; ключи openings_title/add_opening_btn/place_opening_hint не нужны —
  свои подписи есть.
- Именованные аргументы IconChip(icon=, text=, onClick=) — не совпадают с реальной
  сигнатурой, не скомпилировалось бы.
- pushUndo при ВЗВОДЕ режима (а не при установке проёма) — мусорный снимок отмены.

Проверено: check.sh — восемь правил, скобки честным парсером, режим размещения
не задет. Строк не добавлялось (494 без изменений), ядро не менялось.

## Шаг СВ — точки стен снова свободны, замок только настоящим дугам — ВЫПОЛНЕН

Жалоба мастера: «помехи при растягивании стены по точкам, раньше было свободно».
Причина — замок дуги из шага УП: `detectArcs` эвристический, и три-четыре точки,
которые мастер САМ поставил на стене и подровнял (лёгкие изломы одного знака),
он принимал за дугу → вершины запирались, «+» на рёбрах исчезал. Классический
перезажим защиты.

Лечение: замок и скрытие «+» переведены на отдельный `arcLockRuns =
detectArcs(points, minEdges = 6)` — под защиту попадают только НАСТОЯЩИЕ дуги от
шести сегментов подряд (круг, овал, большой машинный сгиб). Подписи «R … · … м» и
плинтус одной длиной остаются на прежней чувствительной эвристике (minEdges = 3) —
там ложное срабатывание безвредно, а информация полезна.

Проверено компилируемым смоуком (kotlinc): ручная стена с тремя точками —
эвристика(3) видит «дугу», замок(6) не видит, точки свободны; круг — 64 ребра под
замком целиком; машинный сгиб стены 4 м — 28 рёбер под замком. Осознанная граница:
сгиб КОРОТКОЙ стены даёт меньше шести сегментов и остаётся редактируемым по точкам
(изломать можно, отмена лечит) — свобода мастера важнее.

Ответы на сопутствующие вопросы мастера зафиксированы в переписке: корректность
расчёта плитки держится на том, что закупка считается с ТОЙ ЖЕ сетки, что нарисована
(плюс 47 тестов ядра); чертёж/схему программа принимает ФОТОГРАФИЕЙ (подложка →
калибровка по известной длине → обводка/автообводка), файлы DXF/SVG — в бэклоге.

## Шаг ТС — тяга за стену в любом месте — ВЫПОЛНЕН

Уточнение мастера к жалобе о помехах: он хватал СТЕНУ и тянул — стена тянулась,
это было удобно; потом стало всплывать «что поставить — дверь?». Разбор истории:
стена тянулась только за «+» в середине ребра (круг 22 px), а добавленный позже
«помощник» с диалогом срабатывал при ПРОМАХЕ мимо этого плюсика — целишься пальцем
в середину, промахнулся на волос → диалог вместо тяги. Диалог убран в шаге ВН;
этим шагом возвращено и усилено само удобство.

В слот убранного диалога (та же позиция в цепочке жестов — приоритеты не тронуты)
встала тяга: тап-драг В ЛЮБОМ месте ребра вставляет вершину ПОД ПАЛЬЦЕМ (проекция
на ребро, round2) и сразу начинает обычный VERTEX-драг — механика один в один как
у «+» (та же вставка, тот же драг, отмена снимает точку). Гарды: roomMode, мастер
проёма закрыт, режим «+ Окно/Дверь» не взведён, не рядом с вершиной; рёбра
настоящих дуг пропускаются (edgeOnArc) — круг/овал остаются цельными. «+» на
середине оставлен как видимая подсказка-афорданс. Осознанная граница: тап по стене
без движения оставляет коллинеарную точку (как и у «+») — безвредно, отмена лечит;
вставка вершины, как и раньше у «+», не сдвигает ключи проёмов дальних стен —
поведение сознательно один в один с существующим.

Проверено: check.sh — восемь правил, скобки честным парсером, гарды на месте,
геометрический смок (палец (2.70; 0.08) над стеной 4 м → точка ровно (2.7; 0.0)).
Ядро не менялось.

## Шаг НХ — находимость: крыльцо, проёмы, свёрнутая плашка, подложка — ВЫПОЛНЕН

Пять вопросов мастера по живым скриншотам — все про «где это лежит», ни одного про
математику. Лечится находимостью, а не объяснениями.

1. **Крыльцо.** Чип «Крыльцо (улица)» был ТУМБЛЕРОМ комнаты (снимает стены, потолок
   и плинтус), а читался как кнопка «добавить крыльцо» — и на скриншоте он к тому же
   оказался включён. Теперь: отдельная кнопка «+ Крыльцо» рядом с «+ Ступени»
   (`addPorch()` → `addStairs(porch = true)`, toLevel = −1), а тумблер переименован
   в «Комната на улице» / «This room is outdoors» — состояние больше не маскируется
   под действие. Заодно `addStairs()` по умолчанию ведёт на СЛЕДУЮЩИЙ этаж, а не в
   никуда: марш и крыльцо теперь различаются с рождения.
2. **Проёмы.** Кнопки «+ Дверь/Окно/Балкон/Вход» были на месте и работали, но связь
   «нажал → тапни по стене» держалась на одной строке подсказки. Добавлена подсветка:
   при взведённом режиме ВСЕ стены обводятся пунктиром цвета Acc2 — палец сразу видит
   мишень.
3. **Плашка чисел** сворачивается в одну строку «20.51 м² · К покупке 77 шт · с
   подрезкой 29» — план получает пол-экрана. Тап по строке разворачивает обратно.
4. **Верхний ряд иконок** (линейка/ножницы/мебель) расширен: свёртка плашки
   (Layers) и — когда загружено фото чертежа — быстрый выключатель подложки (Camera).
   Ровно то «управление и подсветка», о котором просил мастер, в уже привычном месте.
5. **Подложка-план** получила подзаголовок «фото чертежа · калибровка · автообводка»
   прямо в свёрнутом заголовке: раздел перестал выглядеть служебным, и автообводка
   («программа сама нарисует») стала видна не открывая фолд.

Строки `add_porch`, `plan_under_sub` ×4 локали; `outdoor` переписан во всех четырёх.
Проверено: check.sh — восемь правил, 496 ключей ×4, скобки честным парсером,
импорты свёрнутой плашки (Locale/clickable/Alignment) на месте; ядро не менялось.

## Шаг СТЕН — развёртка стен: настоящая раскладка вместо площади — ВЫПОЛНЕН

Главная дыра для мастера-плиточника: стены считались «площадь ÷ площадь плитки ×
запас». В ванной стены — большая часть работы и плитки, и мастер не видел ни
раскладки, ни подрезки, ни того, как плитка ложится вокруг окна.

**Ядро: `core/Walls.kt`.** Ключевая находка — проёмы УЖЕ хранятся в координатах
развёртки (x вдоль стены от угла, y от пола вверх, подтверждено 3D-отрисовкой),
поэтому стена — это прямоугольник «длина × высота» с вырезами-проёмами, и на неё
можно натравить проверенный `TilingEngine`. Никакой второй геометрии: подрезка,
список обрезков, нумерация и отрисовка работают ровно теми же путями, что на полу.
`WallSpec` (длина, высота, проёмы, своя плитка, старт от пола/потолка, симметрия) →
`WallCalc.plan` → `WallPlan` (layout, площадь без проёмов, целые/подрезка, `across`
и `up` — сколько плиток и какие куски по краям, `thinnestMm`, `buyCount(запас)`).
Фаза считается честно: «целая от пола» — линия сетки в y = 0; «целая под потолком» —
линия в (h − плитка) mod шаг; «равные углы» — остаток длины делится пополам.

**Приложение.** `RoomDto.wallTile / wallStartFloor / wallCentered` (в снимок комнаты,
в отмену, в файл проекта). `surfaceLayout("wall-N")` теперь возвращает развёртку —
и панель, и всё, что от неё зависит, получили настоящие числа вместо условных.
Список работ по квартире: стена-плитка отдаёт `buyCount` и число подрезок вместо
деления площадей. В панели «Поверхности» у стены с плиткой: свои размеры плитки и
шва, чипы «Целая от пола / Целая под потолком / Равные углы», ПРЕДПРОСМОТР
развёртки (Canvas: целые — серым, подрезка — тёплым, проёмы дырами), строки «В ряд»
и «Рядов» с краевыми кусками в мм и предупреждение о полоске тоньше 30 мм с советом.

**Проверено.** `WallsTest` — 8 методов; два поймали мою ошибку В ТЕСТЕ: кусок 140 мм,
а не 142 (шов не входит в плитку), и «тонкий край» появляется не при любом остатке —
при остатке меньше шва полоска просто исчезает. Логика движка оказалась права оба
раза. Полный прогон ядра — 63 метода зелёные (kotlinc 2.1.0), check.sh — восемь
правил, 505 ключей ×4 локали, скобки честным парсером, импорты предпросмотра на месте.

Осознанные границы: плитка стен одна на комнату (не на каждую стену отдельно) —
частый случай; бордюр/фриз отдельной строкой пока нет; развёртка не выводится
отдельным листом в PDF (в списке работ есть числа) — следующий заход.

## Шаг ВАР — варианты дизайна А / Б / В — ВЫПОЛНЕН

Вторая дыра из разбора по ролям: дизайнер живёт в «покажу три раскладки, выберем»,
а сравнить два решения можно было только испортив одно.

`DesignVariant` (в ProjectDto, сериализуемый) хранит ТОЛЬКО отделку: плитка, узор,
цвет, вариация, декор, зоны, цвета отдельных плиток, материал, отделка поверхностей,
плитка стен с её фазой, панно. Геометрия, проёмы, мебель, ступени и этажи —
СОЗНАТЕЛЬНО общие: смысл варианта в том, что квартира одна, а решений несколько.

VM: `variants` + `activeVariant`, `addVariant()` (имена А, Б, В… по-русски),
`applyVariant(i)` (меняет отделку, гасит выбор зоны и подсказки, переякоривает),
`updateVariant()` (перезаписать текущий тем, что на экране), `deleteVariant(i)`
с корректным сдвигом активного, `variantLayout(v)` для миниатюр. Всё под pushUndo,
в снимке комнаты, в отмене и в файле проекта.

UI в «Плитка и узор», первым блоком: чипы вариантов + «+ Вариант» + «Обновить этот»
+ удаление; при двух и более — РЯД МИНИАТЮР с настоящей раскладкой каждого варианта
(своя плитка, свой узор, свой цвет; подрезка полупрозрачная), тап по миниатюре
показывает вариант на всю комнату. Это и есть «покажи клиенту А или Б».
Строки `variant*` ×4 локали.

Проверено: ядро 63 метода зелёные (варианты живут в приложении, ядро не менялось),
check.sh — восемь правил, 510 ключей ×4 локали, скобки честным парсером, все
импорты миниатюр и `toArgb` на месте.

Осознанные границы: вариант принадлежит комнате (в квартире у каждой комнаты свои
варианты) — так и задумано; в PDF варианты пока не выводятся отдельными листами.

## Шаг ОДИН — одна программа для всех, без деления на режимы — ВЫПОЛНЕН

**Мастер отверг режим «Просто / Профи» — и был прав.** Деление на уровни это
признание, что программа непонятная: новичок в «простом» упирается в потолок,
профи в «профи» получает свалку, а любой переключатель заставляет человека сначала
объявить, кто он такой. Принцип принят другой: программа одна, любой скачал —
открыл — пользуется и доволен, независимо от уровня и профиля. Курс на будущее:
сложное не прячется за режимом, оно просто не мешает, пока не понадобилось.

Сделано в этом духе:

1. **Одна строка на плане при самом первом открытии** — «Потяни стену — комната
   меняется · тапни плитку — увидишь её рез». Это не обучение и не мастер: строка
   гаснет НАВСЕГДА первым же жестом (`UiPrefs.firstTouch` + `markTouched` в
   `gestureDown`, флаг в SharedPreferences). Новичок получает первый шаг; опытный
   убирает подсказку тем самым движением, которое и так сделал бы. Никто ничего
   не выбирает и ни о чём не заявляет.
2. **Каждый свёрнутый раздел объясняет себя** одной строкой в заголовке — обычными
   словами, а не терминами: «цвет, покраска, зоны с другой плиткой», «своё фото
   плитки», «вставки, бордюр, картина из плиток», «Г-образная, П-образная, готовые
   контуры», «плитка, краска, обои — по каждой поверхности». Панель читается как
   меню: новичок понимает, что внутри, не открывая; профи скользит мимо. Механизм
   уже был (`Fold` умеет подзаголовок) — он просто не использовался.

Строки `first_hint`, `fold_*_sub` ×4 локали. Проверено: check.sh — восемь правил,
516 ключей ×4, скобки честным парсером; ядро не менялось (63 метода зелёные).

## Шаг ПК — показ клиенту и проверка расценок — ВЫПОЛНЕН

**Проверка снятого обвинения.** Я утверждал, что смета считает материал, а не труд —
это оказалось НЕВЕРНО: `Prices` давно содержит `workTileM2 / workWallM2 /
workPaintM2`, поля редактируются в панели цен, а `surfaceCosts()` считает работу по
площади каждой поверхности отдельной строкой рядом с материалом. Ничего не
понадобилось добавлять; ошибка была в моей памяти, а не в программе. Урок тот же,
что и с «дисбалансом скобок»: проверять кодом, а не памятью.

**Показ клиенту.** `ProjectDto.viewOnly` + `viewOnly` во VM. Включённый режим:
в `gestureDown` палец получает только PAN (двигать и приближать), геометрия,
отделка и выбор не трогаются; семь разрушающих функций (`deleteActiveRoom`,
`deleteActiveZone`, `deleteSelectedOpening/Vertex/Cutout/Stairs`,
`deleteActiveLevel`) выходят сразу; при включении гасятся все активные режимы
(рисование, кисть, обводка, калибровка, установка проёма, мастер проёма) — иначе
клиент попал бы в режим без выхода. Флаг ЕДЕТ В ФАЙЛЕ ПРОЕКТА: дизайнер включает
перед отправкой, клиент открывает уже защищённый проект.

Это защита от случайной правки, а не замок: тот же чип «Показ клиенту» рядом с
кнопками обмена выключает режим обратно, а на плане снизу висит метка — никто не
будет гадать, почему план не правится. Строки `view_only`, `view_only_hint` ×4.

Проверено: check.sh — восемь правил, 518 ключей ×4, скобки честным парсером,
7 гардов + гард жеста + запись и чтение флага в проекте; ядро не менялось.

## Шаг ОТЧ — модульный отчёт под ситуацию — ВЫПОЛНЕН

Один отчёт всем не годится: клиенту нужны картинка и деньги, бригаде — подрезка и
номера, в магазин — только количества. Отчёт разобран на блоки.

`ReportOptions` (в ProjectDto, сериализуемый): план, параметры, количества, список
обрезков, номера подрезки, ступени, вся квартира, смета, цены, водяной знак.
`ReportPreset { CLIENT, MASTER, SHOP, FULL }` + `presetOptions(p)` — готовые наборы:
- **Клиенту** — план, вся квартира, смета с ценами; параметры, обрезки и номера сняты
  (техника ему не нужна и пугает).
- **Бригаде** — план, параметры, количества, обрезки, НОМЕРА подрезки, ступени;
  смета и цены сняты (в бригаде цены лишние).
- **В магазин** — параметры, количества, ступени, квартира, смета; чертёж не нужен.
- **Всё** — как было.

`PdfReport.share(..., opts)` заворачивает каждый блок в свой флаг (план, параметры,
материалы+ступени, сводка по квартире, карта подрезки, смета); водяной знак и номера
подрезки дополнительно уважают настройки (`Entitlements.watermark && opts.watermark`,
`vm.showCuts && opts.cutNumbers`). VM: `report`, `applyReportPreset`, `updateReport`,
`reportPreset` (какому пресету точно соответствует текущий набор). Набор ЕДЕТ В
ФАЙЛЕ ПРОЕКТА — отправил проект, отчёт собирается так же.

UI во вкладке «Отчёт» прямо над кнопкой PDF: ряд пресетов, ряд переключателей блоков,
строка-подсказка. Пресет расставляет разом, дальше любой блок включается отдельно.
Строки `report_for*`, `rep_*` ×4 локали.

Проверено: check.sh — восемь правил (правило 2 поймало horizontalScroll без импорта
в ReportTab — добавлен), 532 ключа ×4, скобки честным парсером, все семь флагов
в PDF на месте, смок смысла пресетов; ядро не менялось.

**Дополнение (по вопросу «а может человек сам выбирать»):** да, любой блок
переключается отдельно — и теперь этот выбор становится ЛИЧНЫМ умолчанием.
`ReportOptions.toMask()` / `reportOptionsOf(mask)` упаковывают набор в число,
`UiPrefs.reportMask` хранит его в настройках телефона (−1 = «ещё не выбирал»),
VM пишет туда при каждом изменении и берёт оттуда стартовое значение. Итог:
в проекте лежит набор проекта, в настройках — привычка человека; собрал отчёт
под себя один раз — новые проекты открываются уже с ним. Упаковка проверена
смоком: 200 случайных наборов туда-обратно без потерь.

## Шаг КОМ — обмен между людьми: авторство и журнал — ВЫПОЛНЕН

Вопрос мастера: если программа стоит у разных людей, могут ли они обмениваться
файлами, видеть, кто что сделал, доделывать и возвращаться позже.

**Что уже работало** (шаг ОБ): проект — один файл, кнопки «В файл / Из файла /
Отправить проект», ignoreUnknownKeys для совместимости версий, при совпадении имени
импорт НЕ затирает существующий (суффикс). Внутри файла едет всё, включая
`workStatus` — отметки по работам, то есть прогресс уже путешествовал вместе с
проектом. Не хватало одного: было не видно, ЧЬИ это правки и что человек успел.

**Добавлено.** `HistoryEntry(who, what, at, done, total)` и `ProjectDto.author` +
`history` — журнал носит сам файл, потому что программа работает без интернета и
аккаунтов. `UiPrefs.userName` — имя один раз на телефон (поле «Твоё имя» в разделе
проектов). VM пишет строку журнала при сохранении, выгрузке в файл и импорте
(«получен от Саши»), каждая запись несёт срез прогресса `done/total` по списку
работ (`workDone` считает по `workStatus >= 2`). Журнал ограничен последними 40
записями — файл не пухнет. В панели проектов — блок «Кто что сделал»: последние
шесть строк с именем, действием, датой и прогрессом. `ProjectMeta` получил автора.

Сценарий, который теперь виден целиком: Саша сделал раскладку → «Отправить проект»
→ Витя открыл «Из файла», доделал стены, отметил работы, отправил обратно → Саша
видит «Витя · сохранил · 12/20». Слияния двух одновременных правок НЕТ и не
планируется без сервера: файл приходит целиком, старый проект не затирается —
рядом появляется вторая копия, и человек решает сам.

Проверено: check.sh — восемь правил (правило 2 поймало UiPrefs без импорта в
Panels — добавлен), 538 ключей ×4 локали, скобки честным парсером, смоки на
ограничение журнала и подсчёт прогресса; ядро не менялось.

## Шаг СБР — умный сброс по частям — ВЫПОЛНЕН (место исправлено, см. СБР2)

Сброс разобран на части: «Узор» (поворот 0, смещение 0, привязка ART_CENTER —
лечит «раскладка уехала»), «Отделка» (плитка, цвет, декор, зоны, панно, материал —
геометрия цела), «Лишние точки» (`simplifyRoom`), «Выпрямить стены» (контур →
прямоугольник по своим габаритам; проёмы снимаются честно, они висели на номерах
прежних стен), «Проёмы», «Мебель», «Ступени» — контекстно, только если есть что
убирать. Каждый сброс под `pushUndo`, поэтому нажимать не страшно.

Дубли не плодились: «Крашеное и наставленное» — существующий `resetPlacements`,
«Новый проект» — существующий пункт меню. Три моих ключа (`reset_view`,
`reset_all`, `reset_all_confirm`) уже были заняты старым сбросом — поймала проверка
локалей, свои переименованы.

## Шаг СБР2 — сброс переехал в глобальное меню — ВЫПОЛНЕН

**Мастер забраковал место, и по делу.** Секция сброса стояла В КОНЦЕ панели
«Комната»: её надо было искать прокруткой, а стоя внутри одной панели она выглядела
относящейся ТОЛЬКО к этой панели — хотя сбрасывает узор, отделку и наставленное.
Плюс «Новый проект» задвоился: и в меню «…», и в новой секции.

Исправлено: все пункты сброса перенесены в меню «…» — оно висит в шапке и открыто
из любой панели, поэтому «вернуть как было» не надо искать и оно ни к чему не
привязано визуально. Меню теперь: «Вписать», разделитель, заголовок «Сброс», далее
Узор · Отделка · Упростить контур (если точек больше четырёх) · Выпрямить стены ·
Проёмы / Мебель / Ступени (контекстно) · «Сбросить всё наставленное» (тёплым) ·
серая строка-напоминание «любой сброс снимается отменой», разделитель, «Новый
проект». Дублирующая секция из панели «Комната» удалена целиком, шесть осиротевших
строковых ключей вычищены из четырёх локалей.

Проверено: check.sh — девять правил (правило 4 поймало новый ключ до пуша),
546 ключей ×4 локали без дублей и расхождений, скобки честным парсером, 10 пунктов
сброса в меню, в панели «Комната» сброса не осталось; ядро не менялось.

## Шаг СБ1 — красная сборка: Unresolved reference и правило 9 — ВЫПОЛНЕН

Первая красная сборка за всё время работы, и ровно того класса, о котором
предупреждалось: UI-слой компилируется только в CI, поэтому опечатка в имени
константы живёт до Actions.

**Что упало:** `:app:compileDebugKotlin` — `EditorViewModel.kt:4847 Unresolved
reference 'CENTER'`. В моём `resetPattern()` (шаг СБР) стояло `AnchorMode.CENTER`,
а enum ядра — `{ ART_CENTER, TILE_CENTER, CORNER, FREE }`. Исправлено на
`ART_CENTER` (центр раскладки в центре комнаты — то, что и подразумевалось).
Единственная ошибка сборки; ядро в CI собралось и тесты прошли (`:core:test`
BUILD SUCCESSFUL), предупреждений компилятора ноль.

**Правило 9 в `tools/check.sh`.** Разбирает ВСЕ enum'ы ядра
(`enum class X { A, B, C }` во всех файлах core) и проверяет каждое употребление
`X.CONST` в app: если константы нет в enum — красное. Ловит именно этот класс
ошибок до пуша, как правило 8 ловит незакрытый `when`.

При первом прогоне правило дало ложную тревогу: `StairsFinish.WOOD` попадал под
шаблон для `Finish` (имя кончается на «Finish»). Добавлена граница слова
`(?<![A-Za-z0-9_])` — теперь `StairsFinish.WOOD` не считается `Finish.WOOD`, а
полное имя `com.baremodel.core.StairsFinish.WOOD` по-прежнему проверяется как
`StairsFinish`. Негативный тест: с возвращённым `AnchorMode.CENTER` правило
краснеет, с `ART_CENTER` — зелёное; ложных срабатываний по 8 enum'ам ядра нет.

## Шаг СБ2 — вторая красная сборка: срезанные секции и правило 10 — ВЫПОЛНЕН

**Что упало:** `Panels.kt:1321 Unresolved reference 'ArcsFold'` и то же для
`StairsFold`. Причина — моя правка в шаге СБР2: удаляя секцию сброса из панели, я
вырезал текст ОТ её doc-комментария ДО `private data class FurnPreset(`, а между
ними жили ещё две секции — дуги и ступени. Вызовы остались, определения исчезли.
Обе функции восстановлены полностью (со всеми правками шагов ДГ, СТ, СМ, НХ:
отделка ступеней, крыльцо, «пол под маршем», доски, раскрой, предупреждения).

**Правило 10 в `tools/check.sh`.** Проверяет, что каждая секция, вызванная как
`Xxx(vm)` внутри файла, где-то объявлена. Девять правил этого не ловили: правило 3
проверяет обращения `vm.*`, правило 6 — скобки, а вырезанный блок оставляет файл
синтаксически целым.

Правило доводилось тремя итерациями, каждая — по ложной тревоге:
1) считало только объявления своего файла → ругалось на `EditorStage`/`PanelHost`
   из соседних файлов ПАКЕТА (в Kotlin им импорт не нужен) → теперь собирает
   объявления по всей папке;
2) не понимало функций-расширений `fun BoxScope.EditorStage(...)` → шаблон
   допускает необязательный приёмник перед именем.
Негативный тест: с вырезанным `ArcsFold` правило краснеет, с восстановленным —
зелёное, ложных срабатываний по всему app нет.

Проверено: десять правил зелёные, 546 ключей ×4 локали, скобки честным парсером,
все пять секций панели на месте; ядро не менялось.

## Следующий шаг — не выбран

Бэклог: «точки → авто-контур», DXF/SVG, Play Billing, параметрическая правка дуги,
объём марша в AR.
